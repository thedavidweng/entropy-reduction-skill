...
Scan your own machine through your coding agent and get a classic-Macintosh-style HTML roast report — abandoned projects, dependency graveyards, git shame, AI slop, redacted secret risk. Deterministic Python scanner with hard time/entry budgets; the agent renders the HTML from `references/HTML_REPORT_FORMAT.md`. Reports go to `${TMPDIR:-/tmp}/roast-my-computer/` and are reused on the next run unless you ask to re-scan.
